...
def fly_to(vehicle, targetLocation, groundspeed, startTag, endTag):
    f.write("Flying from: " + str(vehicle.location.global_frame.lat) + "," + str(
    ...
    while vehicle.mode.name == "GUIDED":
        remainingDistance = get_distance_meters(currentTargetLocation, vehicle.location.global_frame)
        f.write("Distance (meters): "+str(remainingDistance)+"\n")
        if remainingDistance < 1:
            f.write("Arrived at waypoint.\n")
            break
        time.sleep(1)
...
if __name__ == '__main__':
    arm_and_takeoff(20)
    locations = [(41.69834,-86.23293),(41.699218,-86.23387),(41.69842,-86.23488)]
    for location in locations:
        f.write("Heading towards "+str(location[0])+" , "+str(location[1])+"\n")
        fly_to(vehicle, LocationGlobalRelative(location[0], location[1], 20), 10, "GOTO-1-START", "GOTO-1-END")
    print(
    "Returning to Launch")
